...
export class BusquedafacturasComponent implements OnInit {
	...
	findFacturas() {
		...
		if (this.idProveedor === null && this.fechaInicio === null && this.fechaFin === null) {
			...
			this._facturaService.getFacturas(null, true, this.fechaInicio, this.fechaFin, this.idProveedor, 2, null).subscribe(response => {
	...
	mostrarModalCambiosFactura(IdFactura: number) {
		...
		this._facturaService.getCambiosFacturaById(IdFactura).subscribe(response => {
			if (response.cambios) {
